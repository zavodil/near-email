...
import { deleteEmail, getAttachment, getMaxDownloadSize, isPaymentKeyMode } from '@/lib/near';
...
export default function EmailView({ email, onDelete, onReply }: EmailViewProps) {
  ...
  function handleAttachmentClick(att: Attachment) {
    ...
    const check = canDownloadAttachment(att);
    if (!check.allowed) {
      alert(check.reason);
      return;
    }
  ...
  // Check if attachment can be downloaded based on current mode limits
  function canDownloadAttachment(att: Attachment): { allowed: boolean; reason?: string } {
    const maxSize = getMaxDownloadSize();
    if (att.size > maxSize) {
      const mode = isPaymentKeyMode() ? 'HTTPS' : 'blockchain';
      const limit = isPaymentKeyMode() ? '18 MB' : '1.1 MB';
      return {
        allowed: false,
        reason: `Attachment too large for ${mode} mode (max ${limit}). Connect with Payment Key for higher limits.`
      };
    }
    return { allowed: true };
  }
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 mb-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {attachments.map((att, idx) => {
              ...
              const downloadCheck = canDownloadAttachment(att);
              const isTooLarge = !downloadCheck.allowed;
              ...
              return (
                <button
                  ...
                  title={isTooLarge ? downloadCheck.reason : undefined}
                ...
                >
                  {isLoading ? (
                    ...
                    <svg className={`w-3.5 h-3.5 ${isTooLarge ? 'text-red-400' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                    </svg>
                  )}
                  <span className={`max-w-[150px] truncate ${isTooLarge ? 'text-gray-500' : 'text-gray-700'}`}>{att.filename}</span>
                  <span className="text-gray-400">({formatSize(att.size)})</span>
                  {isTooLarge && (
                    <span className="text-red-500 text-[10px] font-medium">too large</span>
                  )}
                  {isLazy && !isLoading && !isTooLarge && (
